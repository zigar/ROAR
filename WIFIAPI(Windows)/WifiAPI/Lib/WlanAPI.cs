...
namespace NativeWifi
{
    ...
    public class WlanClient : IDisposable
    {
        ...
        public class WlanInterface
        {
            ...
            public Wlan.WlanAuthCipherPairList SupportedInfraAuthCipherPairs
            {
                get
                {
                    int valueSize;
                    ...
                    Wlan.WlanOpcodeValueType opcodeValueType;
                    Wlan.ThrowIfError(
                        Wlan.WlanQueryInterface(client.clientHandle, info.interfaceGuid, Wlan.WlanIntfOpcode.SupportedInfrastructureAuthCipherPairs, IntPtr.Zero, out valueSize, out valuePtr, out opcodeValueType));
                    try
                    {
                        return (Wlan.WlanAuthCipherPairList)Marshal.PtrToStructure(valuePtr, typeof(Wlan.WlanAuthCipherPairList));
                    }
                    finally
                    {
                        Wlan.WlanFreeMemory(valuePtr);
            ...
            public Wlan.WlanAuthCipherPairList SupportedAdhocAuthCipherPairs
            {
                get
                {
                    int valueSize;
                    ...
                    Wlan.WlanOpcodeValueType opcodeValueType;
                    Wlan.ThrowIfError(
                        Wlan.WlanQueryInterface(client.clientHandle, info.interfaceGuid, Wlan.WlanIntfOpcode.SupportedAdhocAuthCipherPairs, IntPtr.Zero, out valueSize, out valuePtr, out opcodeValueType));
                    try
                    {
                        return (Wlan.WlanAuthCipherPairList)Marshal.PtrToStructure(valuePtr, typeof(Wlan.WlanAuthCipherPairList));
                    }
            ...
            public bool ConnectSynchronously(Wlan.WlanConnectionMode connectionMode, Wlan.Dot11BssType bssType, string profile, int connectTimeout)
            {
                ...
                try
                {
                    Utility.MiscLib.Debug("Connecting ...");
                    WifiAPI.Program.loginfo.Info("Connecting ...");
                    WifiAPI.Program.sw.WriteLine("Connecting ...");
                        ...
                        lock (eventQueue)
                        {
                            while (eventQueue.Count != 0)
                            {
                                ...
                                if (e is WlanConnectionNotificationEventData)
                                {
                                    ...
                                    Utility.MiscLib.Debug("WLAN Event Code " + wlanConnectionData.connNotifyData.wlanReasonCode.ToString());
                                    WifiAPI.Program.loginfo.Info("WLAN Event Code " + wlanConnectionData.connNotifyData.wlanReasonCode.ToString());
                                    WifiAPI.Program.sw.WriteLine("WLAN Event Code " + wlanConnectionData.connNotifyData.wlanReasonCode.ToString());
                                    ...
                                    {
                                        Utility.MiscLib.Debug("wlanConnectionData.notifyData.notificationCode " + ((Wlan.WlanNotificationCodeAcm)(wlanConnectionData.notifyData.notificationCode)).ToString());
                                        WifiAPI.Program.loginfo.Info("wlanConnectionData.notifyData.notificationCode " + ((Wlan.WlanNotificationCodeAcm)(wlanConnectionData.notifyData.notificationCode)).ToString());
                                        WifiAPI.Program.sw.WriteLine("wlanConnectionData.notifyData.notificationCode " + ((Wlan.WlanNotificationCodeAcm)(wlanConnectionData.notifyData.notificationCode)).ToString()); 
                                        switch ((Wlan.WlanNotificationCodeAcm)wlanConnectionData.notifyData.notificationCode)
                                        {
                                            case Wlan.WlanNotificationCodeAcm.ConnectionComplete:
                                                ...
                                                for (int i = 0; i < 5; i++)
                                                {
                                                    if (this.InterfaceState == Wlan.WlanInterfaceState.Connected)
                                                    {
                                                        Utility.MiscLib.Debug("Connected");
                                                        WifiAPI.Program.loginfo.Info("Connected");
                                                        WifiAPI.Program.sw.WriteLine("Connected");
                                                        eventQueue.Clear();
                                                        //queueEvents = false;
                                                        return true;
                                                    }
            ...
            public bool _ConnectSynchronously(Wlan.WlanConnectionMode connectionMode, Wlan.Dot11BssType bssType, string profile, int connectTimeout, StreamWriter sw)
            {
                queueEvents = true;
                try
                {
                        ...
                        lock (eventQueue)
                        {
                            while (eventQueue.Count != 0)
                            {
                                ...
                                if (e is WlanConnectionNotificationEventData)
                                {
                                    WlanConnectionNotificationEventData wlanConnectionData = (WlanConnectionNotificationEventData)e;
                                    Utility.MiscLib.Debug("WLAN Event Code " + wlanConnectionData.connNotifyData.wlanReasonCode.ToString());
                                    sw.WriteLine("WLAN Event Code " + wlanConnectionData.connNotifyData.wlanReasonCode.ToString());
                                    // Check if the conditions are good to indicate either success or failure.
                                    if (wlanConnectionData.notifyData.notificationSource == Wlan.WlanNotificationSource.ACM)
                                    {
                                        Utility.MiscLib.Debug("wlanConnectionData.notifyData.notificationCode " + ((Wlan.WlanNotificationCodeAcm)(wlanConnectionData.notifyData.notificationCode)).ToString());
                                        sw.WriteLine("wlanConnectionData.notifyData.notificationCode " + ((Wlan.WlanNotificationCodeAcm)(wlanConnectionData.notifyData.notificationCode)).ToString());
                                        switch ((Wlan.WlanNotificationCodeAcm)wlanConnectionData.notifyData.notificationCode)
                                        {
                                            case Wlan.WlanNotificationCodeAcm.ConnectionComplete:
                                                //if (wlanConnectionData.connNotifyData.profileName == profile)
            ...
            public bool DisConnectSynchronously(int connectTimeout)
            {
                if (this.InterfaceState == Wlan.WlanInterfaceState.Disconnected)
                {
                    return true;
                }
                queueEvents = true;
            ...
            /// <summary>
            /// Sets the eap profile.add for radius support.kaershaw
            /// </summary>
            public int WlanSetProfileEapXmlUserData(string profileName, string profileXml)
            {
                return Wlan.WlanSetProfileEapXmlUserData(client.clientHandle, info.interfaceGuid, profileName, 0, profileXml, IntPtr.Zero);
            }
